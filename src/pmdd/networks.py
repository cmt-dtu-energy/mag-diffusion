...
def weight_init(shape, mode, fan_in, fan_out) -> None:
...
class AttentionOp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k) -> torch.Tensor:
        w = (
            torch.einsum(
                "ncq,nck->nqk",
                q.to(torch.float32),
    ...
    @staticmethod
    def backward(ctx, dw) -> tuple[torch.Tensor, torch.Tensor]:
...
class PositionalEmbedding(torch.nn.Module):
    def __init__(self, num_channels, max_positions=10000, endpoint=False) -> None:
        super().__init__()
...
class FourierEmbedding(torch.nn.Module):
    def __init__(self, num_channels, scale=16) -> None:
        super().__init__()
...
class SongUNet(torch.nn.Module):
    ...
    Args:
        ...
        augment_dim: Augmentation label dimensionality, 0 = no augmentation.
        ...
        embedding_type: Timestep embedding type: 'positional' for DDPM++,
        'fourier' for NCSN++.
        channel_mult_noise: Multiplier for the dimensionality of the noise vector.
        encoder_type: Encoder architecture: 'standard' for DDPM++,
        'residual' for NCSN++.
        decoder_type: Decoder architecture: 'standard' for both DDPM++ and NCSN++.
        resample_filter: Resampling filter: [1,1] for DDPM++, [1,3,3,1] for NCSN++.
    ...
    def __init__(
        ...
        init = {"init_mode": "xavier_uniform"}
        init_zero = {"init_mode": "xavier_uniform", "init_weight": 1e-5}
        init_attn = {"init_mode": "xavier_uniform", "init_weight": np.sqrt(0.2)}
        block_kwargs = {
...
class VPPrecond(torch.nn.Module):
    ...
    def sigma_inv(self, sigma) -> torch.Tensor:
    ...
    def round_sigma(self, sigma) -> torch.Tensor:
...
class iDDPMPrecond(torch.nn.Module):
    ...
    def alpha_bar(self, j) -> torch.Tensor:
        j = torch.as_tensor(j)
        return (0.5 * np.pi * j / self.M / (self.C_2 + 1)).sin() ** 2

    def round_sigma(self, sigma, return_index=False) -> torch.Tensor:
        sigma = torch.as_tensor(sigma)
        index = torch.cdist(
